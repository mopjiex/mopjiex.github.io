<?php
	...
	if (isset($_POST['id']) && isset($_POST['field_name'])) {
		$upd_query = "UPDATE notebook_br08 SET {$_POST['field_name']} = '{$_POST['field_value']}' WHERE id = {$_POST['id']}";
		mysqli_query($link, $upd_query);
		$query = "SELECT * FROM notebook_br05";
	  ...
	  // output data of each row
		...
		$query = "SELECT * FROM notebook_br08 WHERE id = " .$_POST['id']; 
		$result = mysqli_query($link, $query);
		if (mysqli_num_rows($result) > 0) {
		...
		$query = "SELECT * FROM notebook_br08";
		$result = mysqli_query($link, $query);
		if (mysqli_num_rows($result) > 0) {
	  // output data of each row
			print "<html>
